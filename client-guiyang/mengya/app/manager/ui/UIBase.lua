...
function UIBase:popSelf()
    UIManager:getInstance():hide(self._uiName)
end

function UIBase:destroySelf()
	UIManager:getInstance():destroy(self._uiName)
end

-- @override 获取显示层层级,需要修改默认层级的，覆盖这个函数
function UIBase:getGradeLayerLevel()
    return 3
end
...
function UIBase:playAnimation(csbPath,animName,finishFunc,delay,replay)
    app.UIAnimationManager:getInstance():playAnimationWithParent(nil,self,csbPath,animName,nil,nil,finishFunc,delay,replay)
end

-- onShow时候的动画,如果不需要的话,重写这个方法
function UIBase:playScaleAction()
    ...
    local mask = ccui.Helper:seekNodeByName(self, "__MASK__")
    if mask then
        mask:setOpacity(0)
