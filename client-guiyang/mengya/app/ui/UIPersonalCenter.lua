local csbPath = app.UIPersonalCenterCsb
local super = app.UIBase
local Util = app.Util
local UIManager = app.UIManager
local UITableView = app.UITableView
local UIPersonalCenterLeftItem = app.UIPersonalCenterLeftItem
local UIPersonalCenter = class("UIPersonalCenter", super, function() return app.Util:loadCSBNode(csbPath) end)
...
function UIPersonalCenter:_onBtnBorderChangeClick()
    UIManager:getInstance():show("UIHeadFrameShop")
end
...
function UIPersonalCenter:_onBtnCloseClick()
    UIManager:getInstance():hide("UIPersonalCenter")
end
