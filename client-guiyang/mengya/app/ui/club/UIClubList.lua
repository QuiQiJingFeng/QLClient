local csbPath = app.UIClubListCsb
local super = app.UIBase
local Util = app.Util
local UIManager = app.UIManager
local UITableView = app.UITableView
local UIClubListItem = app.UIClubListItem
local UIClubList = class("UIClubList", super, function() return app.Util:loadCSBNode(csbPath) end)
...
function UIClubList:hideAction()
    ...
    local act2 = cc.CallFunc:create(function()
		UIManager:getInstance():hide("UIClubList")
    end)
