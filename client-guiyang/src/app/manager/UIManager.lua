...
function UIManager:_loadUI(name)
    ...
    -- 获取UI class
    local uiClass = nil
    local path = UI_CONFIG[name]
    if nil ~= config.GlobalConfig.getReplaceUI(name) then -- 优先读取replace的ui
        uiClass = require(config.GlobalConfig.getReplaceUI(name))
    elseif nil ~= path then
...
function UIManager:createMaskLayer(ui)
    ...
    if nil ~= maskUI then
        Logger.error("自动创建Mask出错，UI不能有名字为__MASK__的控件")
    else
...
function UIManager:show(name, ...)
    ...
    game.service.UserEventService:getInstance():createLayerRecord(name, "show", ui:getUIRecordLevel() ,self._curUIName)
...
function UIManager:hideUI(ui)
    ...
    game.service.UserEventService:getInstance():createLayerRecord(ui:getName(), "hide", ui:getUIRecordLevel(), "")
...
cc.exports.bindEventCallBack = function(register, callback, event)
    ...
    local registerCallBack = function(sender, eventType)
        ...
        if event == eventType then
            game.service.UserEventService:getInstance():createButtonRecord(register)
            callback(sender)
        end
    end
...
cc.exports.bindCheckEventCallBack = function(register, callback, event)
    ...
    local registerCallBack = function(sender, eventType)
        ...
        elseif eventType == ccui.TouchEventType.ended then
            game.service.UserEventService:getInstance():createButtonRecord(sender)
            callback(sender)
...
cc.exports.bindTouchEventWithEffect = function(node, callback, touchScale)
    ...
    local registerCallBack = function(sender, eventType)
        ...
        if eventType == ccui.TouchEventType.ended then
            game.service.UserEventService:getInstance():createButtonRecord(node)
            callback(sender, eventType)
        end
    end
...
-- 优点是可以直接访问节点，target.name缺点是同名的节点会被覆盖
