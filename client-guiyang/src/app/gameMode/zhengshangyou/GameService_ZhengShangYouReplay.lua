...
local PlayType = require("app.gameMode.zhengshangyou.core.Constants_ZhengShangYou").PlayType
local Constants = require("app.gameMode.zhengshangyou.core.Constants_ZhengShangYou")
local RoomSetting = require("app.game.ui.RoomSetting")
...
--]]
local GameService_ZhengShangYouReplay = class("GameService_ZhengShangYouReplay", super)

--------------------------------
function GameService_ZhengShangYouReplay:ctor()
    super.ctor(self)
    self._roomUI = nil
    self._roomSeats = {}
    self._players = {}
...
function GameService_ZhengShangYouReplay:initialize(gameScene)
    super.initialize(self, gameScene)
    self._roomProcessor = RoomProcessor.new()
    self._roomProcessor:onGameWaitingStart()
end

function GameService_ZhengShangYouReplay:dispose()
...
function GameService_ZhengShangYouReplay:getRoomUI()
    return self._roomUI;
end

function GameService_ZhengShangYouReplay:getRoomSeats()
    return self._roomSeats;
end

function GameService_ZhengShangYouReplay:getRoomSeat(chairType)
...
-- 设置游戏的最大局数
function GameService_ZhengShangYouReplay:getMaxRoundCount()
    -- TODO : 未实现
...
-- 设置游戏当前局数
function GameService_ZhengShangYouReplay:getCurrentRoundCount()
    -- TODO : 未实现
    Macro.assetFalse(false, "Not implement");
    return 0
end

function GameService_ZhengShangYouReplay:getMaxPlayerCount()
    return self._maxPlayerCount;
end

function GameService_ZhengShangYouReplay:getRoomProcessor()
    return self._roomProcessor
end

function GameService_ZhengShangYouReplay:getPlayerDatas()
	return self._playerDatas;
end

function GameService_ZhengShangYouReplay:getActualPlayerCount()
...
--@return 
--==============================--
function GameService_ZhengShangYouReplay:setRoomPlayerReadyState()
...
-- 当前播放的房间记录
function GameService_ZhengShangYouReplay:getRoomRecord()
    return self._roomRecord
end

-- 墚前播放的对应房间记录的具体的索引
function GameService_ZhengShangYouReplay:getRoundReportIndex()
    return self._roundReportIndex
end

-- @param replayData: com.kodgames.message.proto.battle.MatchPlaybackProto
function GameService_ZhengShangYouReplay:startReplay(followPlayerId, roomRecord, replayData, recordIndex)
...
-- 是否正在播放
function GameService_ZhengShangYouReplay:isReplaying()
    return self._replayPaused == false;
end

-- 暂停播放
function GameService_ZhengShangYouReplay:pauseReplay()
...
-- 继续播放
function GameService_ZhengShangYouReplay:resumeReplay()
...
-- 播放加速
function GameService_ZhengShangYouReplay:increaseReplaySpeed()
    self._currentSpeedIdx = self._currentSpeedIdx + 1 <= #self._replaySpeed and self._currentSpeedIdx + 1 or 1;
end

function GameService_ZhengShangYouReplay:getReplaySpeed()
    return self._replaySpeed[self._currentSpeedIdx]
end

function GameService_ZhengShangYouReplay:getReplaySpeedIdx()
...
function GameService_ZhengShangYouReplay:_setRoomPlayers(localRoleId, players)
...
function GameService_ZhengShangYouReplay:_processStep(isRecover)
...
function GameService_ZhengShangYouReplay:_scheduleProcessStepTask(isRecover, nextTime)
...
-- 取消step处理任务
function GameService_ZhengShangYouReplay:_cancelProcessStepTask()
    if self._processStepTask ~= nil then
...
-- 当前是否在显示等待操作按钮
function GameService_ZhengShangYouReplay:_hasWaitingOperation()
    for i=1, #self._roomSeats do
...
-- 如果有等待操作, 提示操作结果
function GameService_ZhengShangYouReplay:_tryToShowNextOperation(operationWaitTime)
...
function GameService_ZhengShangYouReplay:_needWaitOperation(stepGroup)
...
--@overwrite
function GameService_ZhengShangYouReplay:_convertServerPos( serverPos )
...
--@overwrite
function GameService_ZhengShangYouReplay:_createPlayerProcessor( cPos, player, playerUIObj )
    local clz = nil
    if cPos ==  1 then
...
function GameService_ZhengShangYouReplay:isInReplay()
    return true
end

function GameService_ZhengShangYouReplay:getLastDiscardInfo()
    return self._lastDiscardInfo or {}
end

function GameService_ZhengShangYouReplay:setLastDiscardInfo(info)
...
return GameService_ZhengShangYouReplay