...
local TipsHelper = require("app.gameMode.zhengshangyou.utils.TipsHelper")
...
local CardList_ZhengShangYou = class("CardList_ZhengShangYou")

function CardList_ZhengShangYou:ctor(root)
    self._root = root
    self._uiCards = {}
    self._values = {}
    self._selectedIndex = {}
...
function CardList_ZhengShangYou:setEnable(value)
    self._root:setVisible(value or false)
end


function CardList_ZhengShangYou:onTips()
    ...
    if #values == 1 then
        ...
        if (table.indexof(optValues, "GAME_PLAY_BAO_DAN_BI_DING") or gametype == "GAME_TYPE_ZHENGSHANGYOU") and remainNum == 1 then
...
function CardList_ZhengShangYou:resetTipsCounter()
    self._retryCounter = {
        count = 0,
        retrySplit2Counter = 0,
        retrySplit3Counter = 0,
        retrySplit4Counter = 0,
        retrySplit5Counter = 0,
    } --保持地址引用
end

function CardList_ZhengShangYou:getValues()
    return self._values
end

function CardList_ZhengShangYou:popDownAll()
    for idx, uiCard in ipairs(self._uiCards) do
        uiCard:unselect()
    end
end

function CardList_ZhengShangYou:popReverse(index)
...
function CardList_ZhengShangYou:popUp(index)
...
function CardList_ZhengShangYou:popDown(index)
    -- local index = table.keyof(self._values, value)
    local card = self._uiCards[index]
    if Macro.assertFalse(card.getValue() == value) then
        card:unselect()
...
function CardList_ZhengShangYou:setValues(values)
    self:_deleteAllCard()
...
function CardList_ZhengShangYou:_addAllCardToRoot()
...
function CardList_ZhengShangYou:_createAllCard(values)
...
function CardList_ZhengShangYou:_deleteAllCard()
    self._values = {}
    self._selectedIndex = {}
    for idx, uiCard in ipairs(self._uiCards) do
        uiCard:dispose()
...
function CardList_ZhengShangYou:_deleteCards(cards)
    -- 判断是否含有，没含有就报错
...
function CardList_ZhengShangYou:_resizeContainerSize()
...
function CardList_ZhengShangYou:_onTouchBegan(touch, event)
...
function CardList_ZhengShangYou:_onTouchMoved(touch, event)
...
function CardList_ZhengShangYou:_onTouchEnded(touch, event)
    local index = self:_getCurrentTouchIndex((touch:getLocation()))
    if self._lastTouchIndex ~= index then
        self._lastTouchIndex = index
        self:popReverse(index)
...
function CardList_ZhengShangYou:_convert2Self(p)
    local selfP = self._container:convertToNodeSpace(p)
    return selfP
end

function CardList_ZhengShangYou:_onTouchCanceled()
    Logger.debug("_onTouchCanceled")
end

function CardList_ZhengShangYou:_isTapInView(location)
...
function CardList_ZhengShangYou:_getCurrentTouchIndex(location)
...
function CardList_ZhengShangYou:getSelectedCardValues()
    local ret = {}
...
function CardList_ZhengShangYou:debugSortedValues()
...
-- 目前是删除了所有手牌 后续优化下
function CardList_ZhengShangYou:onDiscard(cards)
    -- 1、恢复所有牌的位置
    -- 2、直接从手牌中删除服务器发来的牌
    -- 3、重新排列位置
...
function CardList_ZhengShangYou:prepareForNextRound()
    self:_deleteAllCard()
    self:setEnable(false)
end

function CardList_ZhengShangYou:toStrings()
    local ret = table.concat(self._values or {}, ",")
    return ret
end

function CardList_ZhengShangYou:dispose()
    self:setTouchEnable(false)
    self:_deleteAllCard()
end

return CardList_ZhengShangYou