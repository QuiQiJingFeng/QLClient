...
function TipsHelper2.get(helper, cardType, retryCounter)
    ...
    elseif cardType == PlayType.POKER_DISPLAY_PAI_SHUANG_SHUNZI then
        ...
        return ret
    elseif cardType == PlayType.POKER_DISPLAY_PAI_SAN_SHUNZI then
        local ret = TipsHelper2._getSanShunZi(helper, retryCounter)
        if ret then
            ret = helper:_getRepeat(ret, 3)
        end
        return ret
...
function TipsHelper2._getSanShunZi(helper, retryCounter)
    if #helper._s3.values == 0 then
        return
    end
    local ret
    ret = helper:_getSerials(helper._s3.values, #helper._o3.values, helper._o3.values[1] + 1, retryCounter)
    if ret ~= nil then
        return ret
    end

    local minValue = helper._o3.values[1] + 1
    local count = #helper._o3.values
    local apart = {}
    for idx, value in ipairs(helper._s3.values) do
        if value >= minValue then
            table.insert(apart, value)
    ...
    -- 从4个中找缺失的
    local attach = {}
    for idx, value in ipairs(helper._s4.values) do
        -- 找到了就插入到apart中
        if value >= minValue then
            table.insert(attach, value)
            print("got a value in duizi " .. value)
    ...
    if #attach == 0 then
        ...
        table.sort(apart, function(v1, v2) return v1 < v2 end)
        -- 再次检查是否符合连续规则
        ret = helper:_getSerials(apart, #helper._o3.values, helper._o3.values[1] + 1, retryCounter)
        return ret
    end
end
