local CardDefines_ZhengShangYou = {}
function CardDefines_ZhengShangYou.getCards(str)
    local cards = {}
    for i = 1, #str do
...
CardDefines_ZhengShangYou.SUIT_COUNT = 4 -- 花色
CardDefines_ZhengShangYou.ONE_SUIT_COUNT = 13 -- 一组
CardDefines_ZhengShangYou.PACK_COUNT = 52 -- 一副

CardDefines_ZhengShangYou.ESuitType = {
    Invalid = -1,
    -- 黑桃
    Spade = 1,
    -- 红心
...
local Spade = CardDefines_ZhengShangYou.ESuitType.Spade
local Heart = CardDefines_ZhengShangYou.ESuitType.Heart
local Club = CardDefines_ZhengShangYou.ESuitType.Club
local Diamond = CardDefines_ZhengShangYou.ESuitType.Diamond

CardDefines_ZhengShangYou.Map = {
    ...
    [2] = { type = Spade, name = "黑桃2", resPath = "poker/surface/s02.png", sortValue = 15 },
    ...
    [15] = { type = Heart, name = "红心2", resPath = "poker/surface/h02.png", sortValue = 15 },
    ...
    [28] = { type = Club, name = "梅花2", resPath = "poker/surface/c02.png", sortValue = 15 },
    ...
    [41] = { type = Diamond, name = "方块2", resPath = "poker/surface/d02.png", sortValue = 15 },
...
function CardDefines_ZhengShangYou.sort(values)
    -- values = values or {}
    Logger.debug("=====in " .. table.concat(CardDefines_ZhengShangYou.convertToSortValue(values), ","))
    -- 没有大小王
    table.sort(values, function(card1, card2)
        local v1 = CardDefines_ZhengShangYou.Map[card1].sortValue
        local v2 = CardDefines_ZhengShangYou.Map[card2].sortValue
        if v1 == v2 then
            return card1 > card2
        else
            return v1 > v2
    ...
    Logger.debug("=====out " .. table.concat(CardDefines_ZhengShangYou.convertToSortValue(values), ","))
    return values
end

function CardDefines_ZhengShangYou.convertToSortValue(cards)
    assert(type(cards) == 'table', 'cards must be a table')
    local ret = {}
    for idx, value in pairs(cards) do
        ret[idx] = CardDefines_ZhengShangYou.Map[value].sortValue
    end
    return ret
end

function CardDefines_ZhengShangYou.getSortValue(value)
    if value then
        local cfg = CardDefines_ZhengShangYou.Map[value]
...
return CardDefines_ZhengShangYou