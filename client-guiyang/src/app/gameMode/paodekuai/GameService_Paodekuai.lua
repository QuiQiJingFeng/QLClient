...
local GameService_Paodekuai = class("GameService_Paodekuai", super)
function GameService_Paodekuai:ctor()
    super.ctor(self)
    self._players = {}
    self._mySelfServerPos = -1
    self._maxPlayerNum = 3
    self._isGameStarted = false
    self._roomProcessor = nil
end

-- called by GameState_Zhajinhua
function GameService_Paodekuai:initialize(gameScene)
...
--@override
function GameService_Paodekuai:_convertServerPos( serverPos )
    -- local basePosition
...
--@override
function GameService_Paodekuai:_createPlayerProcessor( cPos, player, playerUIObj )
    local clz = nil
    if cPos ==  1 then
...
function GameService_Paodekuai:setRoomPlayers( players )
...
function GameService_Paodekuai:dispose()
...
function GameService_Paodekuai:prepareForNextRound(ignorePlayerStatus)
...
-- 设置游戏的最大局数
function GameService_Paodekuai:setMaxRoundCount(count)
    self._maxRoundCount = count
    self._gameScene:setRoundCount(self._currentRoundCount or 0, self._maxRoundCount or 0)
end

-- 设置游戏的最大局数
function GameService_Paodekuai:getMaxRoundCount()
    return self._maxRoundCount
end

-- 设置游戏当前局数
function GameService_Paodekuai:setCurrentRoundCount(count)
    self._currentRoundCount = count
    self._gameScene:setRoundCount(self._currentRoundCount or 0, self._maxRoundCount or 0)
end

-- 设置游戏当前局数
function GameService_Paodekuai:getCurrentRoundCount()
    return self._currentRoundCount
end

function GameService_Paodekuai:isGameStarted()
    return self._isGameStarted
end

function GameService_Paodekuai:_onBattlePlayerInfoSYN(response)
    ...
    -- 亲友圈群主观战需要这个event来关闭结算界面
    local event = {name = "EVT_NEW_GAME_ROUND_BEGIN"}
	self:dispatchEvent(event);
end

function GameService_Paodekuai:_checkClearReport()
	local _tab = {"UICardsInfo_Paodekuai", "UIRoundReportPage2"}
...
function GameService_Paodekuai:_onPlayStepSYN(response)
...
function GameService_Paodekuai:_onMatchResultSYN(response)
    local protocol = response:getProtocol():getProtocolBuf()
...
function GameService_Paodekuai:_onBCFinalMatchResultSYN(response)
    -- assert(false)
    self._finishMatchResultProto = response:getProtocol():getProtocolBuf()
end

function GameService_Paodekuai:getFinishMatchResult()
    return self._finishMatchResultProto;
end

function GameService_Paodekuai:getFinishMachResult()
    return self:getFinishMatchResult()
end


function GameService_Paodekuai:sendPlayStep(playType, cards, datas)
    local roomService = game.service.RoomService.getInstance()
...
function GameService_Paodekuai:setLastDiscardInfo(info)
...
function GameService_Paodekuai:getLastDiscardInfo()
    return self._lastDiscardInfo or {}
end

function GameService_Paodekuai:isInReplay() return false end

return GameService_Paodekuai