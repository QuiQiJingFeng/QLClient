...
local TipsHelper2 = require("app.gameMode.paodekuai.utils.TipsHelper2")
local TipsHelper = {}
...
function TipsHelper:get(selfCards, lastCards, retryCounter)
    ...
    cvt_lastCards = self:_filteA2Serial(cvt_lastCards)
...
function TipsHelper:getCardType(cards)
    ...
    cvt_cards = self:_filteA2Serial(cvt_cards)

    local o1, o2, o3, o4 = self:_analysis(cvt_cards)
    local cardType = nil
    if #o4.values == 1 then
        cardType = PlayType.POKER_DISPLAY_PAI_ZHADAN
    elseif #o3.values >= 2 then
        cardType = PlayType.POKER_DISPLAY_PAI_FEIJI_DAI_CHIBANG
...
function TipsHelper:_cmp(s, o, retryCounter)
    ...
    if s.type == o.type then
        ...
        if retryCounter then
            if retryCounter.retrySplit4Counter > 0 then
...
function TipsHelper:_filteA2Serial(cvt_cards)
    ...
    -- 14 -> 1  15 -> 2
    if isContains2 and isContainsA then
