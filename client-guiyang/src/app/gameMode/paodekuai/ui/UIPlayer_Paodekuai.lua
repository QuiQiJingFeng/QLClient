...
local CD_TIME = 15
local UIPlayer_Paodekuai = class("UIPlayer_Paodekuai", super, function() return kod.LoadCSBNode(csbPath) end)
---@overwrite
function UIPlayer_Paodekuai:ctor(cPos, playerInfo, uiContainer)
    -- Logger.debug("UIPlayer_Paodekuai ctor " .. playerInfo:getShortName())
...
function UIPlayer_Paodekuai:onAddInParent()
...
---@overwrite
function UIPlayer_Paodekuai:init()
    super.init(self)
...
function UIPlayer_Paodekuai:_initElement()
...
function UIPlayer_Paodekuai:_registerCallback()
    bindEventCallBack(self._root, handler(self, self._onSelfClick), ccui.TouchEventType.ended)
end

function UIPlayer_Paodekuai:setEnable(value)
    self:setVisible(value or false)
end

-- call by uigamescene
---@overwrite
function UIPlayer_Paodekuai:updatePlayerInfo(info)
    ...
    -- 更新玩家对话框类型
	local effects = info:getSpecialEffect()
    self:_updateDialogFrame(effects)
end

function UIPlayer_Paodekuai:_updateDialogFrame(effects)
    local dialog = nil
...
function UIPlayer_Paodekuai:setScore(score)
    self._score = score or 0
    self._textScore:setString(score)
end

function UIPlayer_Paodekuai:setCardRemainNum(num)
    self._cardRemainNum = num or 0
    self._textCardRemainNum:setString(self._cardRemainNum)
end

function UIPlayer_Paodekuai:getCardRemainNum()
    return self._cardRemainNum
end

function UIPlayer_Paodekuai:setCounDownEnable(value)
...
local DIS_HAED = 135
function UIPlayer_Paodekuai:onDiscard(cards)
    -- 先删除所有的
...
function UIPlayer_Paodekuai:onGameStarted()
...
function UIPlayer_Paodekuai:prepareForNextRound()
    self._imageReady:setVisible(true)
    self._nodeClock:setVisible(false)
    self._nodeCardRemainNum:setVisible(false)
    self._imagePass:setVisible(false)
...
function UIPlayer_Paodekuai:setPassVisible(value)
...
function UIPlayer_Paodekuai:_onSelfClick()
...
---@overwrite
function UIPlayer_Paodekuai:dispose()
    if self._elemChat then
        self._elemChat:destroy()
    end
    self:_cancelTask()
    self:onDiscard({})
    UIPlayer_Paodekuai.super.dispose(self)
end

function UIPlayer_Paodekuai:_startTask()
...
function UIPlayer_Paodekuai:_cancelTask()
    if self._task then
...
function UIPlayer_Paodekuai:_resetPositionAndAnchorPoint(cPos)
    -- local _Map = 
    -- {
...
function UIPlayer_Paodekuai:getRoomSeat()
    return self
end

function UIPlayer_Paodekuai:getPlayer()
    return self:getPlayerInfo()
end

function UIPlayer_Paodekuai:hasPlayer()
    return self:getPlayerInfo() ~= nil
end

return UIPlayer_Paodekuai