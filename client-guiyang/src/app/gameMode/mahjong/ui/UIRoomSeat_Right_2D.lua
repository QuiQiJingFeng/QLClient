...
function UIRoomSeat_Right:ctor(parent)
	...
	self._root:setPosition(cc.p(x2rx(1005.4), y2cy(134.8)))
	-- 绑定UI控件
	...
	local starty = isClassical and y2by(130 + handOffset) or y2by(120 + handOffset)
	local endy = y2ty(555)
	...
	cardLayout.anchor = cc.p(x2rx(1035), starty);
	...
	cardLayout.discardedAniStart = cc.p(x2rx(950), center.y + offset.y + 30);
	cardLayout.discardedAniStartZOrder = SHOUPAI_Z_ORDER;
	
	cardLayout.discardedLayout.lineSize = DISCARDED_SEQUENCE_LENGTH - 1;
	local adx = isClassical and 750 or 760
	cardLayout.discardedLayout.anchor = cc.p(x2cx(adx + H_DELTA_LIE * 0.5 * DISCARDED_CARD_SCALE), y2cy(250 + V_DELTA_LIE * 0.5 * DISCARDED_CARD_SCALE));
	...
	cardLayout.huaLayout.anchor = cc.p(x2rx(1036 -(H_DELTA / 2 * cardLayout.cardScale + H_DELTA_LIE / 2 * HUA_CARD_SCALE + HUA_SPACE_TO_SHOUPAI)), y2by(180));
	...
	cardLayout.huLayout.anchor = cc.p(x2px(980), y2py(410));
	...
	cardLayout.huLayout.zOrderLineAcvance = 5;
	...
	cardLayout.huStatusPosition = cc.p(x2px(865), y2py(380))
