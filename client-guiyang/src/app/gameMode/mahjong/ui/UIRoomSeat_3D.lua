...
function SubCardLayout:ctor()
	...
	self.lineAdvanceDelta = cc.p(0, 0);	--偏移量随行数增加的修正值
...
function CardLayout:ctor()
	...
	self.cardPromotion = 1;		-- 左右两侧牌的近大远小缩放值
	self.cardPromotion_first = 1;-- 左右两侧牌的近大远小缩放值, 首张牌修正值
	self.discardPromotion = 1;		-- 左右两侧打出的牌的近大远小缩放值
	self.discardPromotion_first = 1;-- 左右两侧打出的牌的近大远小缩放值, 首张牌修正值
	self.discardPromotion_line = 1;-- 左右两侧打出的牌的近大远小缩放值, 首张牌修正值
	self.discardLineRotation = 0;	--左右两边, 打出的牌, 第二行旋转值
	
	self.cardSkewX_first = 0;	--上下两边手牌X轴倾斜角度,第一张
	self.cardSkewX = 0;	--上下两边手牌X轴倾斜角度(每张)
	self.cardSkewX_Get = 0;
	self.groupSkewX = 0;-- 上下两边吃碰杠牌X轴倾斜角度(每张, 但是同组内三张一样, 也就是此值的单位是:度/每三分之一组)
	self.groupSkewX_first = 0;	-- 上下两边吃碰杠牌第一组的倾斜角度
	self.discardSkewX_first = 0;	--上下两边打出牌X轴倾斜角度,第一张
	self.discardSkewX = 0;	--上下两边打出牌X轴倾斜角度(每张)
	...
	self.groupCardColor = cc.c3b(240, 240, 240) -- 打出牌的顶点颜色
	self.discardColor = cc.c3b(245, 245, 245)   -- 吃碰杠的颜色
...
-- @param hasHu: boolean
function CardLayout:getZOrder(hasHu, subCardLayout)
	local delta = 0
	if hasHu then
		delta = self.zOrderHu
	end
	
	if subCardLayout then
		return subCardLayout.zOrder + delta
	else
		return self.zOrder
	end
end
...
-- 弃牌缩放
function UIRoomSeat:DISCARD_SCALE(index)
	index = index or 1
	local lineNumber = math.modf((index - 1) / self._cardLayout.discardedLayout.lineSize)
	local lineIndex =(index - 1) % self._cardLayout.discardedLayout.lineSize
	return self:_getDiscardPromotion(lineIndex, lineNumber) * self._cardLayout.discardedLayout.scale
end
...
function UIRoomSeat:setHuStatus(status)
    local hu_skin = {
        [PlayType.HU_ZI_MO .. 1] = "Icon/icon_suanfen_jiaopao.png",
...
function UIRoomSeat:getShowCardIndex(cardState, cardIndex)
	return 1
end
function UIRoomSeat:createCard(cardState, cardValue, tagIconType, scale, cardIndex, parent)
	if not parent then
		parent = self:getCardParentNode()
	end
	local card = CardFactory:getInstance():createCard3d(self._chairType, cardState, self:getShowCardIndex(cardState, cardIndex), cardValue, tagIconType, scale)
...
function UIRoomSeat:onDiscardACard(cardValue)
	...
	-- 这张牌包含了动画、显示在出牌区所有功能
	local cardIndex = #self._playerProcessor._cardList.discardedCardList % self._cardLayout.discardedLayout.lineSize + 1
	local roomService = game.service.RoomService.getInstance();
	local playerCount = roomService:getMaxPlayerCount();	-- 玩家人数
	
	local card = self:createCard(CardDefines.CardState.Chupai, cardValue, nil, self:DISCARD_SCALE(#self._playerProcessor._cardList.discardedCardList + 1), cardIndex)
	...
	local scale2DiscardSize = cc.ScaleTo:create(0.1, 1)
	local delay = cc.DelayTime:create(0.75)
	local move2DiscardPos = cc.MoveTo:create(0.1, cc.p(discardPlace.pos.x, discardPlace.pos.y))
	...
	local showCardScale = 1
	local show_card = CardFactory:getInstance():createCard3d(CardDefines.Chair.Down, CardDefines.CardState.ShowCard, 1, cardValue, nil, showCardScale)
	...
	if effectChuPaiFangDaTriggle then
		...
		self._mustRunCallback = function(event)
			local scale2DiscardSize = cc.ScaleTo:create(0.05, 1)
...
function UIRoomSeat:runTipsAction(discardLayout, card, scale2DiscardSizeAction, callbackAction)
	-- local show_card = CardFactory:getInstance():createCard3d(self._chairType, CardDefines.CardState.Chupai, 1, card._cardValue, self._cardLayout.discardedLayout.scale)
...
function UIRoomSeat:onDrawCard(card)
	...
	self:_resizeCard(card, self:maxCardNumber())
end
...
function UIRoomSeat:_getRealCardPromotion(index)
	local cardLayout = self:getCardLayout()
	return cardLayout.cardPromotion_first * math.pow(cardLayout.cardPromotion, index)
end

-- 根据近大远小和首张牌的缩放来计算出是多少张牌的位置
function UIRoomSeat:_getRealIndex(index)
	local cardLayout = self:getCardLayout()
	if cardLayout.cardPromotion ~= 1 then
		return cardLayout.cardPromotion_first *(1 - math.pow(cardLayout.cardPromotion, index)) /(1 - cardLayout.cardPromotion)
	else
		return index
	end
end
-- 弃牌真正的缩放比
function UIRoomSeat:_getDiscardPromotion(lineIndex, lineNumber)
	local cardLayout = self:getCardLayout()
	return cardLayout.discardPromotion_first * math.pow(cardLayout.discardPromotion, lineIndex) * math.pow(cardLayout.discardPromotion_line, lineNumber)
end

-- 根据近大远小和首张牌的缩放来计算出是多少张牌的位置(弃牌用)
function UIRoomSeat:_getRealDisCardIndex(index)
	local cardLayout = self:getCardLayout()
	if cardLayout.discardPromotion ~= 1 then
		return cardLayout.discardPromotion_first *(1 - math.pow(cardLayout.discardPromotion, index)) /(1 - cardLayout.discardPromotion)
	else
		return index
	end
end

function UIRoomSeat:_getSubCardLayoutZOrder(subCardLayout, index)
	return subCardLayout.zOrderAdvance
end

function UIRoomSeat:_getCardzOrder(currIndex)
	return self:getCardLayout().zOrderAcvance
end
...
function UIRoomSeat:_getGroupCardPos(cardGroup, index, setPos, cardsNeedPlaceHolder, isMingDa)
	...
	local pt = cc.pAdd(start, cc.pMul(cardLayout.bgAdvance, self:_getRealIndex(currIndex)))
	-- 索引对应的zOrder
	zOrder = zOrder + cardLayout.groupCardzOrderAcvance * currIndex;
	...
	for j = 1, #cardGroup.cards do
		...
		local ptFix = null;
		zOrder = zOrder + self:_getCardzOrder(currIndex)
		...
		if j == 4 then
			...
			if self._chairType == CardDefines.Chair.Left or self._chairType == CardDefines.Chair.Right then
				cardIndex = currIndex + 2
				pt = cc.pAdd(pts_back[2], cc.pMul(cardLayout.gangOffset, self:_getRealCardPromotion(cardIndex)))
			else
			...
			pt = cc.pAdd(pt, cc.pMul(cardLayout.bgAcvanceLie, self:_getRealCardPromotion(cardIndex)))
			-- ptFix = clone(pt)
		end
...
-- @param cardsNeedPlaceHolder: Card[] 当前组内的牌不设置坐标，只是把正确的坐标传出去
-- @return {Card, {pos: point, zOrder: number}}
function UIRoomSeat:_getHandCardPos(cardList, setPos, cardsNeedPlaceHolder, isMingDa)
	local groupCount = #cardList.cardGroups
	local placeHolders = {}
	...
	local offset = self._cardOffset;
	-- 当前牌的基准zorder
	local zOrder = cardLayout:getZOrder(self._hasHuWhenBattleFinished) + groupCount * 3 * cardLayout.zOrderAcvance;
	
	-- 亮牌后的偏移
	local startPoint = cardLayout.anchor
	
	-- 计算手牌开始的位置
	local handStartIndex = self:_getRealIndex(groupCount * 3 + offset)
	local pt = cc.pAdd(startPoint, cc.pMul(cardLayout.bgAdvance, handStartIndex))
	
	local advance = cardLayout.bgAdvance
	
	-- 为了方便处理将摸排的位置调到最后
	local handCards = {}
	for k, v in ipairs(cardList.handCards) do
		if v ~= cardList.lastDrewCard then
			table.insert(handCards, v)
		end
	end
	
	if cardList.lastDrewCard then
		table.insert(handCards, cardList.lastDrewCard)
	end
	
	-- 判断牌是站立还是躺倒来设置间距
	if #handCards > 0 and string.find(handCards[1]._csb, "HP") then
		pt = cc.pAdd(pt, cardLayout.gangOffsetLie)
		advance = cardLayout.bgAcvanceLie
	end
	
	for k, card in ipairs(handCards) do
		local index = k + offset + groupCount * 3
		
		zOrder = zOrder + self:_getCardzOrder(index)
		
		local ptFix = nil
		if card == cardList.lastDrewCard then
			-- 这张处理成摸牌
			ptFix = cc.pAdd(pt, cardLayout.drawCardSpace);
		else
			ptFix = clone(pt)
		end
		...
		if setPos then
			...
			self:_resizeCard(card, index)
		end
		card:changeColor(cardLayout.handCardColor)
		if table.indexof(cardsNeedPlaceHolder, card) ~= false then
			placeHolders[card] = {pos = ptFix, zOrder = zOrder};
		end
		-- 移动坐标去下一张牌
		local promotion = self:_getRealCardPromotion(index)
		pt = cc.pAdd(pt, cc.pMul(advance, promotion))
		
		card:resetCardCsbFor3DIndex(self:getShowCardIndex(card._cardState, index))
	end
	return placeHolders;
end
...
function UIRoomSeat:ManageCardsPositions(cardList, layout, setPos, cardsNeedPlaceHolder,isDrag)
	...
	if #cardList.handCards > 0 then
		if self:getChairType() == CardDefines.Chair.Down then
			isMingDa = cardList.handCards[1]._cardState == CardDefines.CardState.Pengpai
		else
	...
	-- 因为产品的特殊要求手牌的算法为了方便计算改成直接累加坐标来算
	table.foreach(self:_getHandCardPos(cardList, setPos, cardsNeedPlaceHolder, isMingDa), function(index, val)
...
--[[	重设手牌的缩放值, 主要用于确定索引之后, 展示近大远小的效果
]]
function UIRoomSeat:_resizeCard(card, index)
	local layout = self:getCardLayout()
	local scale = 1
	
	local skewX = 0;
	
	local baseScale =(string.find(card._csb, "HP") or string.find(card._csb, "GP")) and self:GROUP_SCALE() or self:CARD_SCALE()
	scale = baseScale * self:_getRealCardPromotion(index)
	skewX = self:_getRealIndex(index) * layout.cardSkewX + layout.cardSkewX_first
	
	if self._huStatus and self._huStatus:isVisible() then
		skewX = 0
	end
	card:setScale(scale)
	card:setSkewX(skewX)
...
function UIRoomSeat:_resizeSubCard(card, index, subCardLayout, cardLayout)
	local lineNumber = math.modf((index - 1) / subCardLayout.lineSize)
	local lineIndex =(index - 1) % subCardLayout.lineSize
	
	local zOrder = cardLayout:getZOrder(self.hasHuWhenBattleFinished, subCardLayout) + lineNumber * subCardLayout.zOrderLineAcvance + lineIndex * self:_getSubCardLayoutZOrder(subCardLayout, lineIndex + 1)
	local scale = subCardLayout.scale * self:_getDiscardPromotion(lineIndex, lineNumber)
	if subCardLayout.cardSkewX then
		card:setSkewX(subCardLayout.cardSkewX)
	end
...
function UIRoomSeat:_ManageSubLayoutPositions(cards, subCardLayout, setPos, cardsNeedPlaceHolder)
	...
	local offset = clone(startOffset);
	
	local roomService = game.service.RoomService.getInstance();	-- 
	local playerCount = roomService:getMaxPlayerCount();	-- 玩家人数
	-- 修正后的坐标
	local offsetFix = cc.p(0, 0)
	for i = 1, #cards do
		-- 步进位置
		if i ~= 1 then
			local lineIndex =(i - 1) % subCardLayout.lineSize
			local lineNumber = math.modf((i - 1) / subCardLayout.lineSize)
			local realPromotion = self:_getDiscardPromotion(lineIndex, lineNumber)
			-- 换行的话重置偏移,不换行则向下累加
			if lineIndex == 0 then
				offset = clone(startOffset)
			else
				-- 左右玩家牌的间距是靠下的一张牌跟本牌的距离算出来的,右边是从下往上摆放
				-- 所以右边的玩家,算距离要算上一张牌的(为了使左右两边对称的特殊处理,其实看不出太多区别)
				local advancePromotion = realPromotion
				if self._chairType == CardDefines.Chair.Right then
					advancePromotion = self:_getDiscardPromotion(lineIndex - 1, lineNumber)
				end
				offset = cc.pAdd(offset, cc.pMul(subCardLayout.advance, advancePromotion))
			end
			--行偏差
			local lineOffset = nil
			if self._chairType == CardDefines.Chair.Left or self._chairType == CardDefines.Chair.Right then
				-- 左右玩家弃牌每行的偏差固定(因为左右的牌同行大小一样,所以不用额外算两张牌各一半的倍率)
				lineOffset = cc.pMul(cc.pMul(subCardLayout.lineAdvance, realPromotion), lineNumber)
			else
				--上下玩家的每行的偏差是等比数列
				local cardLayout = self:getCardLayout()
				local realLineIndex = 0
				if lineNumber ~= 0 then
					realLineIndex =(1 - math.pow(cardLayout.discardPromotion_line, lineNumber)) /(1 - cardLayout.discardPromotion_line) * cardLayout.discardPromotion_line
				end
				
				lineOffset = cc.pMul(subCardLayout.lineAdvance, realLineIndex)
			end
			offsetFix = cc.pAdd(offset, lineOffset)
			
		end
		
		local card = cards[i];
		local zOrder, scale = self:_resizeSubCard(card, i, subCardLayout, cardLayout)
		if setPos then
			if table.indexof(cardsNeedPlaceHolder, card) == false then
				card:setPosition(cc.pAdd(subCardLayout.anchor, offsetFix));
				card:setZOrder(zOrder);
				card:setScale(scale)
			end
		else
			placeHolders[card] = {pos = cc.pAdd(subCardLayout.anchor, offsetFix), zOrder = zOrder};
		end
	end
...
function UIRoomSeat:onWaitingOperation(playType, selectCallback, setting)
	...
	if Macro.assertFalse(opBtn:isVisible() == false) then
...
function UIRoomSeat:getColor(k, isSameJudge)
	...
	elseif k == "groupCards" then
		color = self._cardLayout.groupCardColor
	else
	...
	for i1, v1 in ipairs(v._cardList.cardGroups) do
		if v1.cardState == CardDefines.CardState.Pengpai then
			...
			for i2, v2 in ipairs(v1.cards) do
				...
				v2:changeColor(self:getColor("groupCards", isSameJudge), true)
			end
		elseif v1.cardState == CardDefines.CardState.ChiPai then
			-- 吃
			for i2, v2 in ipairs(v1.cards) do
				if v2:getCardValue() == cardValue then
					if v2:getCardValue() == cardValue then
