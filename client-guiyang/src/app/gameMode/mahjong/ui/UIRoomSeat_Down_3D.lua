...
function UIRoomSeat_Down:onDiscardACard(cardValue,isTing)
	...
	local index = #self._playerProcessor._cardList.discardedCardList + 1
	...
	local card = self:createCard(CardDefines.CardState.Chupai, cardValue, nil, self._cardLayout.discardedLayout.scale, cardIndex)
	...
	local scale2 = cc.ScaleTo:create(0.1, self:DISCARD_SCALE(index))
...
function UIRoomSeat_Down:onInsertDrewCard(lastDrewCard)
	...
	local seq = nil
	if effectValues.effect_XieChaPai then
		local rotate0 = cc.RotateTo:create(0.05, 0, 0)
		local rotate30 = cc.RotateTo:create(0.05, 30, 30)
		seq = cc.Sequence:create(moveUp, rotate30, move, rotate0, moveDown, callback)
	else
...
-- 挂载提示
function UIRoomSeat_Down:updateCardTips()
...
function UIRoomSeat_Down:ManageCardsPositions(cardList, layout, setPos, cardsNeedPlaceHolder, isDrag)
	local res = self.super.ManageCardsPositions(self, cardList, layout, setPos, cardsNeedPlaceHolder)
	if not isDrag then
		self:updateCardTips()
	end
	return res
...
function UIRoomSeat_Down:_createDraggingShadow(draggingCard)
	...
	local card = CardFactory:getInstance():createCard3d(self._chairType, CardDefines.CardState.Shoupai, 7, draggingCard._cardValue, self._playerProcessor._cardList:getCardCornerTypes(draggingCard._cardValue), self:CARD_SCALE());
	card:disable();
