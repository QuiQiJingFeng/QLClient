...
local UIElemNoticeSave = class("UIElemNoticeSave")

function UIElemNoticeSave:ctor()
    self._filepath = cc.FileUtils:getInstance():getAppDataPath().."/TMP_CONFIG"
    self._key = "NoticeLastSelect"
end

local _noticeSaver = nil
function UIElemNoticeSave:getInstance()
    if not _noticeSaver then
        _noticeSaver = UIElemNoticeSave.new()
    end
    return _noticeSaver
end

function UIElemNoticeSave:getLastSelected()
    -- 默认显示第一页
    local index = nil
...
function UIElemNoticeSave:saveLastSelected(index)
    local __json = nil
    local buffer = nil
    if cc.FileUtils:getInstance():isFileExist(self._filepath) then
        local file = io.open(self._filepath, "rb")
...
function NOTICE_CONFS.fromConfig()
    NOTICE_CONFS.data = {
        MAKE_CONF(NOTICE_CONFS.ELEM_TYPE.IMAGE, "", "art/activity/jkyxgg.jpg", nil),
    }
    return NOTICE_CONFS.data
end
 
-- 通过gmt配置来处理显示，现在未完成
function NOTICE_CONFS.fromProtocol(finalNotice)
    if #finalNotice == 1 then 
    NOTICE_CONFS.data = {
        MAKE_CONF(NOTICE_CONFS.ELEM_TYPE.IMAGE, finalNotice[1].title, finalNotice[1].pageImg, nil),
    }
    elseif  #finalNotice == 2   then
    NOTICE_CONFS.data = {
        MAKE_CONF(NOTICE_CONFS.ELEM_TYPE.IMAGE, finalNotice[1].title, finalNotice[1].pageImg, nil),
        MAKE_CONF(NOTICE_CONFS.ELEM_TYPE.IMAGE, finalNotice[2].title, finalNotice[2].pageImg, nil),
    }
    else
    NOTICE_CONFS.data = {
        MAKE_CONF(NOTICE_CONFS.ELEM_TYPE.IMAGE, finalNotice[1].title, finalNotice[1].pageImg, nil),
        MAKE_CONF(NOTICE_CONFS.ELEM_TYPE.IMAGE ,finalNotice[2].title, finalNotice[2].pageImg, nil),
        MAKE_CONF(NOTICE_CONFS.ELEM_TYPE.IMAGE, finalNotice[3].title, finalNotice[3].pageImg, nil),
    }
   end
    return NOTICE_CONFS.data
end
...
local UIElemNotice = class("UIElemNotice")
...
function UIElemNotice.extend(self, tabs, indicator)
    local t = tolua.getpeer(self)
    if not t then
        t = {}
        tolua.setpeer(self, t)
    end
    setmetatable(t, UIElemNotice)
	self:_initialize(tabs, indicator)
    return self
end

-- 初始化子组件，相关参数看上面
function UIElemNotice:_initialize(tabs, indicator)
...
function UIElemNotice:_makeTDData(index)
    if index >#self._conf then 
        return 
    end 
...
-- 根据配置加载界面
function UIElemNotice:load(conf, callbacks)
    -- #conf or max 3?
    self:_clear()
    ...
    -- 默认选中第一个
    local last = UIElemNoticeSave:getInstance():getLastSelected()
...
-- 选中事件
function UIElemNotice:select(index)
    ...
    UIElemNoticeSave:getInstance():saveLastSelected(index)
...
function UIElemNotice:_connectComponent(index)
    -- 同步指示器
    if self._indicator then
...
-- 配置文件当做孩子传入
UIElemNotice.NOTICE_CONFS = NOTICE_CONFS
