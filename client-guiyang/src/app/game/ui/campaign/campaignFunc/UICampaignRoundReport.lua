...
local UICampaignRoundReport = class("UICampaignRoundReport", super, function() return kod.LoadCSBNode(csbPath) end)
...
function UICampaignRoundReport:ctor()
    -- 存放其他玩家的panel位置，这个界面不继续的话不会销毁，下次再进入，不重置计算会失误
    self._panelPositionArray = {}
    self._callback = nil
end

function UICampaignRoundReport:init()
    --其他玩家面板
    ...
    --自己的分数详细列表
    self._listScore = ListFactory.get(seekNodeByName(self, "listScore", "ccui.ListView"), initDetailList, setListData, "UICampaignRoundReport")
    -- 自己对于这场比分的表情
...
function UICampaignRoundReport:needBlackMask()
    return true
end

function UICampaignRoundReport:onShow(...)
    local args = { ... }
    ...
    for k, v in ipairs(roundReportData.matchResults) do
        if game.service.LocalPlayerService.getInstance():getRoleId() == v.roleId then
            ...
            local totalPoint = v.pointInGame
            if totalPoint >= 0 then
                self._mbTextMyScore:setFntFile("ui/art/font/font_Arena.fnt")
            else
                self._mbTextMyScore:setFntFile("ui/art/font/font_js2.fnt")
            end
            local isPositive, scoreString = UtilsFunctions.getScoreWithOperator(totalPoint)
            self._mbTextMyScore:setString(scoreString)
            self:setLocalPlayerBridEmojiStatus(totalPoint, huData, roundReportData.isHuang)
        else
            ...
            local totalPoint = v.pointInGame
            UtilsFunctions.setScoreWithColor(panelPlayer.bmTextScore, totalPoint)
    ...
    self._btnContinue:setEnabled(roomGrade ~= 0)
...
function UICampaignRoundReport:_onCampaignNextRound()
    local gameService = gameMode.mahjong.Context.getInstance():getGameService();

    gameService:prepareForNextRound()
    UIManager:getInstance():destroy("UICampaignRoundReport")
end

function UICampaignRoundReport:_adjustPanelPlayerPosition(playerNum)
...
function UICampaignRoundReport:_resetPlayerPanelPosition()
    for i, pos in ipairs(self._panelPositionArray) do
        self._panelOtherPlayers[i]:setPosition(pos)
    end
end

function UICampaignRoundReport:onHide()
    ...
    game.service.CampaignService.getInstance():dispatchEvent({name = "EVENT_CAMPAIGN_RANK_DISPLAY"})  
end


function UICampaignRoundReport:_registerCallBack()
...
--取消匹配
function UICampaignRoundReport:_onBtnReturn(sender)
    local goldService = game.service.GoldService.getInstance()
    if goldService.giftCache then
        UIManager.getInstance():show("UIGoldLeaveTip", goldService.giftCache.needRoundNextMallPoint, goldService.giftCache.rewardMallPoint)
    else
...
--取消匹配成功后的回调
function UICampaignRoundReport:_onBtnContinue()
    local goldService = game.service.GoldService.getInstance()
    local roomGrade = goldService:getlastSelectRoomGrade()
    if roomGrade == 0 then
...
function UICampaignRoundReport:_isAutoShowSharePage()
    local goldService = game.service.GoldService.getInstance()
...
--转换结算数据成客户端需要显示的数据
function UICampaignRoundReport:_changeMatchEventToData(matchResult, isHuang)
    -- 胡牌类型
    ...
    for _, event in ipairs(matchResult.events) do
        if event.score.type == PlayType.DISPLAY_MULTIPLE then
            table.foreach(event.subScores, function(k, v)
                times = v.point    
            end
            )
        end
    ...
    for _, event in ipairs(matchResult.events) do -- ResultEventPROTO
                ...
                if subscore.point ~= 0 then
                    ...
                    if operatorCharacter == "x" then
                        for k, v in pairs(eventMap) do
                            v.labelScore = v.labelScore * subscore.point * times
                        ...
                        insertEvent({
                            index = index - 1000,
                            labelScore = subscore.point * targets * flag * times,
                            target = targets,
        ...
        elseif event.combinedPoint ~= 0 then -- 不知道这样改会不会影响贵阳的，有问题再说吧
            insertEvent({
                index = index,
                labelScore = event.eventPoint * times,
...
function UICampaignRoundReport:_setListScore(listEventData)
...
local happyEmojiPath = "art/campaign/campaignRound/img_logo2.png"
local sadEmojiPath = "art/campaign/campaignRound/img_logo3.png"
local huangZhuangImgPath = "art/gold/z_hz.png"
local victoryImgPath = "art/gold/z_sl.png"
function UICampaignRoundReport:setLocalPlayerBridEmojiStatus(totalPoint, huData, isHuangZhuang)
...
function UICampaignRoundReport:setCallback(callback)
    self._callback = callback
end

function UICampaignRoundReport:getCallback()
    return self._callback
end

return UICampaignRoundReport