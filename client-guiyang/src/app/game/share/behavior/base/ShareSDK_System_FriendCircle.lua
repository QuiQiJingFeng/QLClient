...
local sharURLToSystem_FriendCircle = function(url, shareInfo, shareIcon, shareApp)
...
local shareToSystem_FriendCircle = function(fileName)
	...
	if device.platform ~= 'android' or currentVersion:compare(Version.new("4.6.0.0")) < 0 then
		...
		game.plugin.Runtime.shareImageToWXTimeLine(
			fileName,
			""
		)
	end
...
return {
    shareUrlFunc = sharURLToSystem_FriendCircle,
    sharePicFunc = shareToSystem_FriendCircle
}